...
$(document).ready(function(){
	...
	if(selectInput!="undefined"){
		if(selectInput=='0'){
			$('select[name="filter_and_printState_EQ_S"]').find("option[value='0']",navTab.getCurrentPanel()).attr("selected",true);
		}else if(selectInput=='1'){
			$('select[name="filter_and_printState_EQ_S"]').find("option[value='1']",navTab.getCurrentPanel()).attr("selected",true);
		}else if(selectInput=='2'){
			$('select[name="filter_and_printState_EQ_S"]').find("option[value='2']",navTab.getCurrentPanel()).attr("selected",true);
		}else if(selectInput=='3'){
			$('select[name="filter_and_printState_EQ_S"]').find("option[value='3']",navTab.getCurrentPanel()).attr("selected",true);
		}else if(selectInput=='4'){
			$('select[name="filter_and_printState_EQ_S"]').find("option[value='4']",navTab.getCurrentPanel()).attr("selected",true);
		}else if(selectInput=='5'){
			$('select[name="filter_and_printState_EQ_S"]').find("option[value='5']",navTab.getCurrentPanel()).attr("selected",true);
		}
...
//将选中的状态放入input
$('select[name="filter_and_printState_EQ_S"]',navTab.getCurrentPanel()).change(function (){
...
//	var form = $("#pagerFindForm",navTab.getCurrentPanel());
	$.ajax({
		...
		url : "${path}/report/xreportPrintTask!confirmBatchPrintTask.action",
		success : function(data) {
...
<div class="pageHeader">
	<form id="pagerFindForm" onsubmit="if(this.action != '${path}/report/xreportPrintTask!listPrintTask.action'){this.action = '${path}/report/xreportPrintTask!listPrintTask.action' ;} ;return navTabSearch(this);" action="${path}/report/xreportPrintTask!listPrintTask.action" method="post">
	<div class="searchBar">
		<table class="pageFormContent">
			<tr>
				...
				<td>
					<label>打印公司：</label>
					<select id="warehouseId" name="filter_and_printcompanyId_EQ_S" style="width:193px;margin-left:5px; margin-top: 5px;">
