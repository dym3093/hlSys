...
public class JSONUtil {
	...
	public static String list2JSON(List list,String[] exclude) 
		...
		jsonRoot.put("rows",JSONArray.fromObject(list, jsonconfig));
		return jsonRoot.toString();
  	}
	 ...
	 * @param response
	 */
	public static void returnJSON(List list) 
		throws IOException {
		HttpServletResponse response = ServletActionContext.getResponse() ;
		JSONUtil.print(list2JSON(list));
  	}
	 ...
	 * @param exclude 字符串数组对象，如 {"id","userid"} 转换后将不包含这些字段
	 */
	public static void returnJSON(List list, String[] exclude) 
		throws IOException {
		HttpServletResponse response = ServletActionContext.getResponse() ;
		JSONUtil.print(list2JSON(list, exclude));
  	}
