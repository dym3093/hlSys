...
<%@ page import="org.hpin.common.util.StaticMehtod,
                 ...
                 java.util.*,org.hpin.base.accessories.entity.TawCommonsAccessoriesConfig" %>
...
<%
 ...
 String id=StaticMehtod.nullObject2String(request.getParameter("id"));
...
<script type="text/javascript">
 ...
 function confirmForm(){ 
    	...
    	frm.action="${path}/accessories/pages/upload.jsp?appId="+appId+"&filelist="+filelist+"&id="+id+"&startsWith="+startwith;
    	frm.submit();
 	}
 function deleteFile()
  {
    var frm=document.tawCommonsAccessoriesForm;
    	...
    	if(temp.length==0){
   		...
   		if(confirm("您确定要删除上传的文件吗？")){
   			location.href="remove.jsp?appId=<%=appId%>&removeid=" + temp.substring(0,temp.length-1) + "&filelist=" + filelist + "&id=<%=id%>&startsWith=" + startUrlStr;
  		}
  }   
...
<script type="text/javascript" src="${path}/scripts/util/iframe.js"></script>
</head>
...
<% 
    ...
    for(int i=0;i<fileList.size();i++){
      ...
      filelist=filelist+",'"+file.getAccessoriesName()+"'";
		...
		path = path + file.getAccessoriesName();
...
<script type="text/javascript">
   try{
   	 parent.document.getElementById('<%=id%>').value= "<%=filelist%>";
