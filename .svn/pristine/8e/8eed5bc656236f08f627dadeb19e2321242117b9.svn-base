...
#moduleDiv div a{
	...
	<body>
		...
		<form action="bigRole!updateBigRole.action" method="post" id="_form"
			class="form">
			<input type="hidden" name="bigRole.id" value="${bigRole.id }" />
			<table width="100%">
				<tr>
					...
					<td width="70%">
						<input type="text" name="bigRole.roleName" value="${bigRole.roleName }" maxlength="50"
				...
				<tr>
					...
					<td>
						<input type="text" name="bigRole.roleCode" value="${bigRole.roleCode }" maxlength="50"
				...
				<tr>
					...
					<td>
						<textarea rows="10" cols="55" name="bigRole.desc">${bigRole.desc }</textarea>
					</td>
				</tr>
				<tr>
					<td class="label" style="text-align: right;">
					...
					<td>
						<div id="moduleDiv">
							<c:forEach items="${brrList }" var="brr" varStatus="status1">
								<div>${brr.role.name }<input type="hidden" value="${brr.roleId }"><a href="javascript:void(0)" onclick="deleteUser(this ,  'moduleDiv' , 'moduleIds')">删除</a></div>
							</c:forEach>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div id="userDiv">
							<c:forEach items="${userBigRoles }" var="userRole" varStatus="status1">
								<div>${userRole.user.userName}<input type="hidden" value="${userRole.userId }"><a href="javascript:void(0)" onclick="deleteUser(this ,  'userDiv' , 'userIds')">删除</a></div>
							</c:forEach>
						</div>
					</td>
				</tr>
