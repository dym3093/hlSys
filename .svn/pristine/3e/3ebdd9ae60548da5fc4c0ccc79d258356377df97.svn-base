...
<script type="text/javascript" language="javascript">
	...
	function yestoprint(){
		...
		$.ajax({
			...
			cache :false,
			dataType: "json",
			url: "${path}/report/xreportPrintTask!confirmPrintTaskReloadWeb.action",
...
function showUnPrintBatchDialog(){
	$.pdialog.open("../report/xreportPrintBatch!listUnPrintBatch.action?id=${erpPrintTask.id}&printTaskNo=${erpPrintTask.printTaskNo}", "listUnPrintBatch", "增加打印批次",
...
function deletePrintBatch(){
	...
	alertMsg.confirm('<span style="color:#FF0000;">确认删除【'+count+'】条打印任务?</span>', {
        okCall: function(){
        	$.ajax({
				url: '${path}/report/xreportPrintBatch!removePrintBatchFromPrintTask.action',
				method: 'post',
				cache: false,
				dataType: 'json',
...
<div class="pageHeader" style="background:white">
	...
	<div class="searchBar">
			...
			<tr>
				<td><label>任务编号：</label> 
					<input type="text" name="printTaskNo" value="${XreportPrintTask.printTaskNo}" readonly="readonly"/>
					<input type="hidden" id="taskId" name="id" value="${XreportPrintTask.id}"/>
					<input type="hidden" name="printState" value="${XreportPrintTask.printState}"/>
				</td>
				<td><label>任务名称：</label> 
					<input type="text" name="taskName" value="${XreportPrintTask.taskName}" readonly="readonly">
				</td>
			...
			<td>
					<label>批次数量：</label> 
					<input type="text" name="batchNum" value="${XreportPrintTask.batchNum}" readonly="readonly"/>
				</td>
				<td align="center">
					<label>创建时间：</label> 
					<input type="text" name="createTime" value="${XreportPrintTask.createTime}" readonly="readonly"/>
				</td>
			...
			<tr>
            	<td>
            		<label>报告总数：</label> 
					<input type="text" name="batchNum" value="${XreportPrintTask.reportCount}" readonly="readonly"/>
                </td>
                <td>
		...
		<tbody>
			<c:forEach items="${page.results}" var="erpPrintBatch" varStatus="status">
				<tr target="sid_user" rel="${erpPrintBatch.id }">
					...
					<td align="center"><hpin:id2nameDB id="${erpPrintBatch.branchCompanyId}" beanId="org.hpin.statistics.briefness.dao.CustomerRelationshipDao"/></td>
