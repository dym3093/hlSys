...
(function($){
	$.fn.activityNameSelect=function(settings){
		...
		// 默认值
		settings=$.extend({
			url:"../scripts/ws/wsTransport/activityName.min.js",
			cycleName:null,
			activityName:null,
			dist:null,
			nodata:null,
		...
		var init=function(){
			...
			$.each(activityName_json.activityNamelist,function(i,cycleName){
				if(cycleName.p=="0"){
					...
					temp_html+="<option value='"+cycleName.p+"'>转运前</option>";	
				}else if(cycleName.p=="2"){
					temp_html+="<option value='"+cycleName.p+"'>转运中</option>";	
				}else if(cycleName.p=="3"){
					temp_html+="<option value='"+cycleName.p+"'>理赔前</option>";	
				}else if(cycleName.p=="4"){
					temp_html+="<option value='"+cycleName.p+"'>理赔中</option>";	
				}else if(cycleName.p=="5"){
					temp_html+="<option value='"+cycleName.p+"'>理赔完毕</option>";	
				}else if(cycleName.p=="6"){
					temp_html+="<option value='"+cycleName.p+"'>服务完毕</option>";	
				}else{
