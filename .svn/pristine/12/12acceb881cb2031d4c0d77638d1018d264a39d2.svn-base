 ...
 * 多级联动下拉框控件
 */
hpin.ComboBox = function(){
	var dataUrl = "${app}/hpin/hpinSystemDictType!xsearch.action&dictId=";
	var dataStore = new Ext.util.MixedCollection();
	var Options = hpin.form.Options;
	return {
		/**
		 * 执行联动
		 */
		...
		getData : function(dataId,subEl,url){
			//先在dataStore中查找
  			...
  			Ext.Ajax.request({
				method:'get',
               	url: hpin.appPath+url+dataId,
               	success: function(req){             		
					if(!req || req.responseText=="")return;
  					var ds = hpin.JSONDecode(req.responseText);
