	...
	<div class="searchBar">
	  <table class="pageFormContent num1_tab">
		...
		<tr>
			...
			<td>
				<input type="text" name="filter_and_reportLaunchDate_GE_T" value="${ actionMap.filter_and_reportLaunchDate_GE_T }"  id="d7113" style="width: 170px;" onFocus="WdatePicker({minDate:'2010-01-01',maxDate:'#F{$dp.$D(\'d7114\')}'})" class="required"  readonly="readonly" />
				<a class="inputDateButton" href="javascript:;">选择</a>
			</td>
			<td></td>
			<td><label>到：</label></td>
			<td>
				<input type="text" name="filter_and_reportLaunchDate_LE_T" value="${ actionMap.filter_and_reportLaunchDate_LE_T }"   id="d7114" style="width: 170px;" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'d7113\')}'})" readonly="readonly" />
				<a class="inputDateButton" href="javascript:;">选择</a>	
			</td>
			<td></td>
			<td></td>
				...
				<table class="list" width="100%" layoutH="150" id = "exportTable"> 
				   <thead>
					<tr>
					    ...
					    <th nowrap="nowrap" export = "true" columnIndex = "8" columnChName = "备注">备注</th>			    
					    <th nowrap="nowrap" export = "false" columnIndex = "9" columnChName = "操作">操作</th>			    
					</tr>
				  </thead>
				  <tbody>
					...
					<c:forEach items="${page.results }" var="members" varStatus="status">
					  <tr>
						...
						<td align="center" style="width: 120px">
							<div class="panelBar">
								<ul class="toolBar" >
										<li><a class="edit" href="javascript:void(0)" onclick="changeInfo('${members[9]}')"
											 rel="edit" title="修改"><span>修改</span></a></li>
                        		</ul>
							</div>
						</td>
					  </tr>
...
<script>
	...
	//变更用于单行
	function changeInfo(ids) {
		var navTabId = navTab._getTabs().filter('.selected').attr('tabid');
		navTab.openTab("modifyCustomerInfo",
				"../reportdetail/reportPassBackShenyou!toModifyCustomerInfo.action?id=" + ids
						+ "&navTabId=" + navTabId, {
					title : "变更",
					fresh : false,
					data : {}
