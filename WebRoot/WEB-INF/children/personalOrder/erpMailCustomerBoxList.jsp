...
<script type="text/javascript" language="javascript">
	...
	function fnSave(){
		...
		var dataJson ="[";
		$("#mailCustomer1 tr td").find("#groupId").each(function(index,data){
...
//			ids+=$.trim($(data).text())+",";
		...
		$.ajax({	
			...
			cache :true,
			dateType:"json",
			url: "${path}/children/erpOrderInfo!insertMailCustomerBoxInfo.action",
			...
			success: function(data){
				...
				if(s.count!="error"){
					...
					$("#mailCustomer1 tr td").find("#smailNo").each(function(index,data){
						$.trim($(data).text(mailNo));
						$.trim($(data).parent().next().text("已处理"));
					});
...
<div class="pageHeader" style="margin-top: 10px;">
	<form id="pagerFindForm" rel="pagerForm" onsubmit="if(this.action != '${path}/children/erpOrderInfo!insertMailCustomerBoxInfo.action'){this.action = '${path}/children/erpOrderInfo!insertMailCustomerBoxInfo.action' ;} ;return navTabSearch(this);" 
		action="${path}/children/erpOrderInfo!insertMailCustomerBoxInfo.action" method="post"	>
	<div class="searchBar">
		<table class="pageFormContent" id="tableId">
			<tr>
				<td >
					<label style="width: 120px;">寄客户采样盒快递号：</label> 
					<input type="text" id="mailNo1" value="${mailNo}" />
					<input type="text"  name="filter_and_ids_LIKES_S"  id="ids" value="${ids}" hidden="hidden"/>
				</td>
				<td>
         			<div class="buttonActive"><div class="buttonContent"><button type="button" id="bt_save" onclick="fnSave()">保存</button></div></div>
	...
	<div class="panelBar">
		<ul class="toolBar" >
			...
			<web:exportExcelTag pagerFormId="pagerFindForm"
								...
								informationTableId="groupOrderTable1"
								fileName="ExpressCustomerInfo"></web:exportExcelTag> 
		</ul>
		<jsp:include page="/common/pagination.jsp" />
	</div>	
	<table class="list" width="100%" layoutH="170" id="groupOrderTable1"> 
			<thead>
			<tr>
