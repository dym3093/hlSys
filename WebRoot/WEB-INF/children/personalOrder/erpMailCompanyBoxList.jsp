...
<script type="text/javascript" language="javascript">
	...
	function fnSave(){
		var mailNo=$("#mailNo2").val();
		if($.trim(mailNo)==""){
			alertMsg.info("请输入快递单号！");
			return;
		}
		var dataJson ="[";
		$("#mailCustomer2 tr td").find("#groupId").each(function(index,data){
	...
	//		ids+=$.trim($(data).text())+",";
		...
		$.ajax({	
			...
			cache :true,
			dateType:"json",
			url: "${path}/children/erpOrderInfo!insertMailCompanyBoxInfo.action",
			...
			success: function(data){
				...
				if(s.count!="error"){
					...
					$("#mailCustomer2 tr td").find("#smailNo").each(function(index,data){
						$.trim($(data).text(mailNo));
						$.trim($(data).parent().next().text("已送检"));
					});
...
<div class="pageHeader">
	...
	<div class="searchBar">
		<table class="pageFormContent" id="tableId">
			<tr>
				<td >
					<label style="width: 120px;">寄基因公司快递号：</label> 
					<input type="text" value="${mailNo}" id="mailNo2"/>
	...
	<div class="panelBar">
		<ul class="toolBar" >
			...
			<web:exportExcelTag pagerFormId="pagerFindForm"
								...
								informationTableId="groupOrderTable2"
								fileName="ExpressGeneCompany"></web:exportExcelTag>
		</ul>
		<jsp:include page="/common/pagination.jsp" />
	</div>	
	<table class="list" width="100%" layoutH="170" id="groupOrderTable2"> 
			<thead>
			<tr>
