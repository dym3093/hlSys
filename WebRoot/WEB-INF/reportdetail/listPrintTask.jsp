...
$(document).ready(function(){
	...
	$.ajax({	//初始化页面时套餐
		...
		success: function(data){
			var selectWareId = $('#printCompanyInput',navTab.getCurrentPanel()).val();
...
$("#warehouseId").change( function() {	//状态下拉框
	var selectVal=$("#warehouseId option:selected").val();
	$("#printCompanyInput").val(selectVal);
});
	...
	alertMsg.confirm('<span style="color:#FF0000;">确认删除【'+printTaskNo+'】打印任务?</span>', {
			okCall: function(){
				$.ajax({
					...
					method: 'post',
					cache: false,
					data: {'ids':printTaskId,"printTaskNo":printTaskNo},
...
<div class="pageHeader">
	<form id="pagerFindForm" onsubmit="if(this.action != '${path}/reportdetail/erpPrintTask!listPrintTask.action'){this.action = '${path}/reportdetail/erpPrintTask!listPrintTask.action' ;} ;return navTabSearch(this);" action="${path}/reportdetail/erpPrintTask!listPrintTask.action" method="post"  rel="pagerForm">
	<div class="searchBar">
		<table class="pageFormContent">
			<tr>
				...
				<td>
					...
					<select id="printState" name="filter_and_printState_EQ_S" style="width: 194px; margin-left: 5px; margin-top: 6px;">
				...
				<td>
					<label>打印公司：</label>
					<select id="warehouseId" name="printCompanySel" style="width:194px;margin-top:6px; margin-left: 5px;">
					<input id="printCompanyInput" type="hidden" name="filter_and_printcompanyId_EQ_S" value="${filter_and_printcompanyId_EQ_S}"/>
				</td>
			</tr>
			...
			<tr>
				<td>
					<label>下载状态：</label> 
					<select id="downloadStatus" name="filter_and_downloadStatus_EQ_S" style="width: 194px; margin-left: 4px; margin-top: 6px;">
						 <option value="">---请选择---</option>
	...
	<table class="list" width="100%" layoutH="185" id="exportExcelTable"> 
		<thead>
			<tr>
				...
				<th width="80"  export= "true" columnEnName = "printState" columnChName = "打印状态" >打印状态</th>
				<th width="80"  export= "true" columnEnName = "downloadStatus" columnChName = "下载状态" >下载状态</th>
				<th width="80"  export= "true" columnEnName = "operation" columnChName = "操作" >操作</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${page.results}" var="erpPrintTask" varStatus="status">
					<tr target="sid_user" rel="${erpPrintTask.id }">
						...
						<td nowrap="true" align="center" title="${erpPrintTask.printTaskNo}">${erpPrintTask.printTaskNo}</td>
						<td nowrap="true" align="center">${erpPrintTask.printCompany}</td>
						<td nowrap="true" align="center" title="${erpPrintTask.address}">
						...
						<td nowrap="true" align="center">${erpPrintTask.expressRecipient}</td>
						<td nowrap="true" align="center">${erpPrintTask.telephone}</td>
						<td nowrap="true" align="center">${erpPrintTask.expressNo}</td>
						<td nowrap="true" align="center"><fmt:formatDate value="${erpPrintTask.createTime }" pattern="yyyy-MM-dd"/></td>
						<td nowrap="true" align="center"><fmt:formatDate value="${erpPrintTask.expectTime }" pattern="yyyy-MM-dd"/></td>
