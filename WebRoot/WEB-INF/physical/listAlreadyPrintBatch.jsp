...
function dj(){
	...
	$.pdialog.open("../report/xreportPrintBatch!isPrintBatch.action?ids="+paramStr, "lookPrintTask.jsp", "打印任务",{width:800,height:600});
}
...
<div class="pageHeader">
	<form id="pagerFindForm" onsubmit="if(this.action != '${path}/report/xreportPrintBatch!llistAlreadyPrintBatch.action'){this.action = '${path}/report/xreportPrintBatch!listAlreadyPrintBatch.action' ;} ;return navTabSearch(this);" action="${path}/report/xreportPrintBatch!listAlreadyPrintBatch.action" method="post"  rel="pagerForm" id="pagerFindForm">
	<div class="searchBar">
		<table class="pageFormContent">
				<tr>
					<td><label>批次号：</label> <input type="text"
						name="filter_and_printBatchNo_LIKE_S"
						value="${filter_and_printBatchNo_LIKE_S}" /></td>

					<td><label>套餐：</label> <input type="text"
						name="filter_and_combo_LIKE_S" value="${filter_and_combo_LIKE_S}" />
					</td>

					<td><label>支公司：</label> <input type="text"
						name="filter_and_branchCompany_LIKE_S"
						bringbackname="customerRelationShip.branchCommany"
						value="${filter_and_branchCompany_LIKE_S}" readonly="readonly" />
						<!-- <input type="hidden" id="id" name="conference.branchCompanyId" bringbackname="customerRelationShip.id" value=""/> -->
						<a class="btnLook"
						href="${ path }/report/xreportPrintBatch!lookCustomerRelationShip.action"
						lookupGroup="customerRelationShip">查找带回</a> <img
						src="${path}/images/clear.png" title="清除公司信息"
						id="customerRelation" style="padding-top: 6px;" /></td>
				</tr>

				<tr>
					<td><label>省：</label> <input type="hidden" name="provinceTwo"
						value="${provinceTwo}" /> <input type="hidden" name="cityTwo"
						value="${cityTwo}" /> <%-- <input type="text" name="filter_and_address_LIKE_S" value="${filter_and_address_LIKE_S}"/>  --%>
		 			 ...
		 			 </select> --%> <select id="provinceAlready"
						name="filter_and_province_EQ_S" style="margin: 5px; width: 193px;">
							<option value="">---请选择---</option>
					</select></td>
					<td><label>市</label> <select id="cityAlready"
						name="filter_and_city_EQ_S" style="margin: 5px; width: 193px;">
							<option value="">---请选择---</option>
					</select></td>
				</tr>
				<tr>
					<td><label>起始日期：</label> <input type="text"
						name="filter_and_createTime_GEST_T" id="d1" style="width: 170px;"
						onFocus="WdatePicker({minDate:'2010-01-01',maxDate:'#F{$dp.$D(\'d2\')}'})"
						readonly="readonly" value="${filter_and_createTime_GEST_T}" /><a
						class="inputDateButton" href="javascript:;">选择</a></td>
					<td><label>结束日期：</label> <input type="text"
						name="filter_and_createTime_LEET_T" id="d2" style="width: 170px;"
...
<div class="pageContent">
	...
	<table class="list" width="100%" layoutH="170" id="exportExcelTable"> 
			...
			<tr>
				...
				<th  export = "true" columnEnName = "branchCompanyId" id2NameBeanId="org.hpin.statistics.briefness.dao.CustomerRelationshipDao" columnChName = "支公司" >支公司</th>
		...
		<tbody>
			<c:forEach items="${page.results}" var="xreportPrintBatch" varStatus="status">
					<tr target="sid_user" rel="${xreportPrintBatch.id }">
						...
						<td align="center">
						<%-- 
						<a title="批次信息" target="navTab" width="1000"  href="${path}/report/xreportPrintBatch!printBatchDetail.action?id=${xreportPrintBatch.id}&printBatchNo=${xreportPrintBatch.printBatchNo}">${xreportPrintBatch.printBatchNo}</a>
						--%>
						${xreportPrintBatch.printBatchNo} 
						</td>
						<td align="center"><hpin:id2nameDB  beanId="org.hpin.base.region.dao.RegionDao" id="${xreportPrintBatch.province }"/></td>
						<td align="center"><hpin:id2nameDB  beanId="org.hpin.base.region.dao.RegionDao" id="${xreportPrintBatch.city }"/></td>
						<td align="center"><hpin:id2nameDB id="${xreportPrintBatch.branchCompanyId}" beanId="org.hpin.statistics.briefness.dao.CustomerRelationshipDao"/></td>
		
						<%-- <td align="center">	${xreportPrintBatch.events}</td> --%>
						<td align="center">	${xreportPrintBatch.combo}</td>
						<td align="center">	${xreportPrintBatch.count}</td>
					</tr>
				</c:forEach>
			</tbody>
	</table>
