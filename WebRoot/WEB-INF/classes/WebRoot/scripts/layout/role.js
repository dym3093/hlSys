...
Ext.onReady(function(){
    ...
    var newNodeBtn = form.addButton({text:'保存子节点',hidden:true}, function(){
    	...
    	Ext.lib.Ajax.request(
    		...
    		{success: function(){
    			...
    			msg("创建成功");openPanel('helpPanel');
    		 },
    		 failure: function(){unmask();msg("创建失败")}
    		},
    		data
    	);
    });
    var modifybtn = form.addButton({text:'保存修改',hidden:true}, function(){
    	...
    	Ext.lib.Ajax.request(
    		...
    		{success: function(){
    			...
    			msg("修改成功");openPanel('helpPanel');
    		 },
    		 failure: function(){unmask();msg("修改失败")}
    		},
    		data
    	);
    });
    ...
    var flowNodeLoader = new Ext.tree.TreeLoader({
            //dataUrl:hpin.appPath+'/test/wf.json',
...
function pregrid(){
	...
	if(dupeRecords.length>0){
		msg(dupeRecords.length+"条重复的记录已被覆盖:"+dupeRecords.toString());
	}
...
function crtSubRoles(){
	...
	Ext.lib.Ajax.request(
		...
		{success: function(){
		 ...
		 failure: function(){unmask();msg("提交失败")}
		},
		data
	);
}
	...
	Ext.MessageBox.confirm("确认:","您确定删除项目吗？",function(btn){
		if(btn=="yes"){
			...
			Ext.Ajax.request({
				 ...
				 success: function(){gridds.reload();msg("删除子角色成功。");},
	...
	Ext.MessageBox.prompt("修改子角色名称:","请输入子角色的新名称",function(btn,text){
		if(btn=="ok"){
			...
			Ext.Ajax.request({
				 ...
				 failure: function(){msg("修改名称失败。");},
				 params:"id="+subroleId+"&newName="+text
			});
		}
	});	
}
...
function resetDistincter(){
    ...
    mgr.update(hpin.appPath + '/role/tawSystemSubRoles.do?method=getRoleFiler&id='+selectNode.attributes.workflowFlag);
	$("subRolesForm").reset();
...
function setTeamLeader(){
	...
	var jsondata = userViewer.getNodeData(node);
		Ext.Ajax.request({
    		method:"get",
    		url:hpin.appPath+"/role/tawSystemSubRoles.do?method=xSetLeader&id="+subRoleId+"&userId="+jsondata.id,
    		success: function(){
