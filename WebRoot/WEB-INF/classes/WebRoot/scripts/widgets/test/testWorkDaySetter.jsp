...
<link rel="stylesheet" type="text/css" href="${path}/styles/default/lunar-calendar.css"></link>
...
<div class="wds-header"><h1>${hpin:a2u("工作日设置工具")}</h1></div>
...
<div>
  <div id="wds-sample">
    <div>${hpin:a2u("工作日:")}</div>
    <div class="wds-box"> </div>
    
    <div>${hpin:a2u("非工作日:")}</div>
    <div class="wds-box" id="snwd"> </div>
    
    <div>${hpin:a2u("有变化的日期:")}</div>
...
<table border="0" cellspacing="0" cellpadding="5">
	<tr><td colspan="2">${hpin:a2u("将所选日期设置为:")}</td></tr>
	<tr>
		<td colspan="2"><input class="radio" type="radio" name="dateConfig" value="0"
			onchange="wds.onEditConfig(this.value)" />${hpin:a2u("使用默认设置")}</td>
		<td></td>
	</tr>
	<tr>
		<td colspan="2"><input class="radio" type="radio" name="dateConfig" value="1"
			onchange="wds.onEditConfig(this.value)" />${hpin:a2u("非工作日")}</td>
	</tr>
	<tr>
		<td colspan="2"><input class="radio" type="radio" name="dateConfig" value="2"
			onchange="wds.onEditConfig(this.value)" />${hpin:a2u("非默认工作时间")}</td>
	</tr>
	<tr>
		<td>${hpin:a2u("从")}</td>
		<td>${hpin:a2u("到")}</td>
	...
	<tr>
		...
		${hpin:a2u('覆盖下级地域的本日设置')}
		<br/><br/><input type="button" value="${hpin:a2u('保存')}" onclick="wds.log();" /></td>
	</tr>
</table>
</div>
</div>
...
<script type="text/javascript">
    ...
    var wds = new hpin.WorkDaySetter("dateSetter",config);
