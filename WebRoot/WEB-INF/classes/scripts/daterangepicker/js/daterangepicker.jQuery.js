...
jQuery.fn.daterangepicker = function(settings){
  ...
  $.datepicker.setDefaults($.datepicker.regional['zh-CN']);
	...
	$(this).each(function(){
		...
		options.beginName = rangeInput.attr('beginName')?rangeInput.attr('beginName'):options.beginName;
		options.endName = rangeInput.attr('endName')?rangeInput.attr('endName'):options.endName;
		var dateStart = rangeInput.attr('dateStart')?$.trim(rangeInput.attr('dateStart')):'';
		var dateEnd = rangeInput.attr('dateEnd')?$.trim(rangeInput.attr('dateEnd')):'';
		if(dateStart!='' || dateEnd!=''){
			rangeInput.val(dateStart+' '+options.rangeSplitter+' '+dateEnd);
		}
		
		//custom datepicker options, extended by optionss
		var datepickerOptions = {
			onSelect: function() { 
					...
					if(rangeInput.length == 2){
						...
						rangeInput.parent().find('[name='+options.beginName+']').val(rangeA);
						rangeInput.parent().find('[name='+options.endName+']').val(rangeB);
					}
		...
		rangeInput.parent().append('<input type="hidden" name="'+options.beginName+'" value="'+dateStart+'">');
		rangeInput.parent().append('<input type="hidden" name="'+options.endName+'" value="'+dateEnd+'">');
		...
		var resetBtn = jQuery('<button class="resetDone ui-state-default ui-corner-all">'+ options.resetButtonText +'</button>')
		.click(function(){
			rangeInput.val('');
			rangeInput.parent().find('[name='+options.beginName+']').val('');
			rangeInput.parent().find('[name='+options.endName+']').val('');
			rp.find('.ui-datepicker-current-day').removeClass('ui-datepicker-current-day');
			rp.find('.range-start').datepicker('setDate', '' ); 
