    ...
    return {
        init : function(){
            ...
            var tree = new Tree.TreePanel({
                ...
                animate:true, 
                autoScroll:true,
                loader: new Tree.TreeLoader({dataUrl:'../assign/assignPriv!treeResource.action'}),
                enableDD:true,
           ...
           tree.on('beforeload', function(node) {
				tree.loader.dataUrl = 'assignPriv!treeResource.action?parentId=' + node.id;
			});
            ...
            var tree2 = new Tree.TreePanel({
                ...
                loader: new Ext.tree.TreeLoader({
                    dataUrl:'assignPriv!treeAssignResource.action'
                }),
                containerScroll: true,
                enableDD:true,
                ...
                var treeMenu = new Ext.menu.Menu({
				     items : [{
				        ...
				        handler : function(){
				        	...
				        	//执行删除
                             Ext.Ajax.request({
                                 url:'assignPriv!deletePriv.action?ownerId='+ownerId+'&assignType='+assignType+'&id='+node.id,
                                 success:function(request){
                                    var json = eval("("+request.responseText+")");
		                        	if(json.error == 1){
		                        		Ext.Msg.alert("执行结果：","权限删除失败，"+json.errorMsg+"!");
		                        	}else{
		                        		node.remove();
            ...
            tree2.on('beforeload', function(node) {
				tree2.loader.dataUrl = 'assignPriv!treeAssignResource.action?ownerId='+ownerId+'&assignType='+assignType+'&parentId=' + node.id;
			});
			...
			tree2.on('nodedrop', function(e){   
		         if(e.point=='append'){
		         	...
		         	//执行授权
                    Ext.Ajax.request({
                        url:'assignPriv!addAssignPriv.action?ownerId='+ownerId+'&assignType='+assignType+'&resIds='+ids,
                        success:function(request){
                        	var json = eval("("+request.responseText+")");
                        	if(json.error == 1){
                        		Ext.Msg.alert("执行结果：","权限分配失败，"+json.errorMsg+"!");
                        	}else{
