	...
	return {
		...
		init : function(){
			...
			layout = new Ext.BorderLayout(document.body, {        
		        ...
		        center: {
		            titlebar: false, autoScroll:true, hideTabs:config.hideSearchPanel!=false,
    			...
    			if (form.isValid()) {
					form.submit({
						failure: function(form,x) {
							var response = hpin.JSONDecode(x.response.responseText);
    	 ...
    	 * @param {Ext.menuItem} menuItem 菜单项
    	 */
    	addmenu : function(nodeType,menuItem){
    		var newmi = this.treeCtxMenu.add(menuItem);
