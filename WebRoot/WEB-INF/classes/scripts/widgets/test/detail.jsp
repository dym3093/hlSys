...
<%@page import="org.hpin.sheet.base.task.ITask" %>
<%@page import="org.hpin.sheet.base.model.BaseMain" %>
<%@page import="org.hpin.sheet.commonfault.task.impl.CommonFaultTask" %>
<%@page import="org.hpin.commons.system.session.form.TawSystemSessionForm" %>
<%
TawSystemSessionForm sessionform = (TawSystemSessionForm) request
				.getSession().getAttribute("sessionform");
String taskStatus = StaticMehtod.nullObject2String(request.getParameter("taskStatus"));
String isAdmin = StaticMehtod.nullObject2String(request.getParameter("isAdmin"));
...
System.out.println("sendUserId>>>>>>"+sendUserId);
 ...
 String taskName = org.hpin.base.util.StaticMehtod.nullObject2String(request.getAttribute("taskName"));
  String ifInvokeUrgentFault = org.hpin.base.util.StaticMehtod.nullObject2String(request.getAttribute("ifInvokeUrgentFault"));
 System.out.println("taskName>>>>>>"+taskName);
 System.out.println("ifInvokeUrgentFault>>>>>>"+ifInvokeUrgentFault); 
%>


<script type="text/javascript" src="${path}/scripts/widgets/TabPanel.js"></script>
...
Ext.onReady(function() {
	...
	var tabs = new hpin.TabPanel('sheet-detail-page', tabConfig);
	...
	hpin.util.appendPage("sheet-deal-content",url2);	
	<%}%>
...
function forceOperation(obj){
	...
	if(obj == 1){
	     var url2="commonfault.do?method=showPhaseAdvicePage&sheetKey=${sheetMain.id}&piid=${sheetMain.piid}&taskId=${taskId}&taskName=advice&operateFlag=driveForward&operateRoleId=${operateRoleId}&TKID=${TKID}&preLinkId=${preLinkId}";
             hpin.util.appendPage("sheet-deal-content",url2,true);
	}
   }
  ...
  <div id="sheetinfo">
	...
	<table class="listTable"> 
	  ...
	  <tr>
		...
		<td class="content"><web:id2nameDB id="${sheetMain.mainNetSortOne}" beanId="ItawSystemDictTypeDao"/></td>
		<td class="label"><bean:message bundle="commonfault" key="commonfault.mainNetSortTwo"/></td>
		<td class="content"><web:id2nameDB id="${sheetMain.mainNetSortTwo}" beanId="ItawSystemDictTypeDao"/></td>
	  </tr>
      <tr>
		<td class="label"><bean:message bundle="commonfault" key="commonfault.mainNetSortThree"/></td>
		<td class="content"><web:id2nameDB id="${sheetMain.mainNetSortThree}" beanId="ItawSystemDictTypeDao"/></td>
		
		<td class="label"><bean:message bundle="commonfault" key="commonfault.mainFaultResponseLevel"/></td>
		<td class="content"><web:id2nameDB id="${sheetMain.mainFaultResponseLevel}" beanId="ItawSystemDictTypeDao"/></td>
      ...
      <tr>
		...
		<td class="content"><web:id2nameDB id="${sheetMain.mainFaultDiscoverableMode}" beanId="ItawSystemDictTypeDao"/>
		</td>
		<td class="label"><bean:message bundle="commonfault" key="commonfault.mainSendMode"/></td>
		<td class="content"><web:id2nameDB id="${sheetMain.mainSendMode}" beanId="ItawSystemDictTypeDao"/></td>
	  </tr>
	  
      <tr>
        <td class="label"><bean:message bundle="commonfault" key="commonfault.mainEquipmentType"/></td>
		<td class="content"><web:id2nameDB id="${sheetMain.mainEquipmentType}" beanId="ItawSystemDictTypeDao"/></td>
		<td class="label"><bean:message bundle="commonfault" key="commonfault.mainEquipmentFactory"/></td>
		<td class="content"><web:id2nameDB id="${sheetMain.mainEquipmentFactory}" beanId="ItawSystemDictTypeDao"/></td>
	  </tr>
      ...
      <tr>
		...
		<td class="content"><web:id2nameDB id="${sheetMain.toDeptId}" beanId="tawSystemAreaDao"/></td>
	  </tr>
      ...
      <td class="label"><bean:message bundle="commonfault" key="commonfault.mainFaultGenerantTime"/></td>
		...
		<td class="content"><web:id2nameDB id="${sheetMain.mainIfAffectOperation}" beanId="ItawSystemDictTypeDao"/></td>
	  </tr>	
...
<c:if test="${!empty sheetMain.mainApplySheetId}">	  
	 ...
	 <!--   <tr>
		...
		<tr>
		    ...
		    <td  colspan='5'>
		    <web:attachment name="sheetMain" property="sheetAccessories" 
...
<div class="sheet-deal">
	...
	<%if(taskName.equals("FirstExcuteHumTask")){ %>
	  <%--if(taskStatus.equals("8") && (task!=null && task.getTaskOwner().equals(userId))){--%>
		<select id="dealSelector" onchange="javascript:hpin.util.appendPage('sheet-deal-content',this.value,true);">
...
<script type="text/javascript">
	...
	try{
		...
		hpin.util.appendPage("sheet-deal-content",url);		
	});
</script>
...
<c:if test="${sheetMain.status==0}"> 
 ...
 <%if(isAdmin.equals("true")){%>
    ...
    <input type="button" title="${hpin:a2u('执行该操作，工单将进入强制归档状态，其他人不能在处理工单')}" value="<bean:message bundle="sheet" key="button.forceHold"/>" onclick="forceOperation(1);">
    <input type="button" title="${hpin:a2u('执行该操作，工单将进入强制作废状态，其他人不能在处理工单')}" value="<bean:message bundle="sheet" key="button.forceNullity"/>" onclick="forceOperation(3);">
 ...
 <div id="advice" style="display:block">
     <input type="button" title="${hpin:a2u('执行该操作，工单将进入作废状态，其他人不能在处理工单')}" value="<bean:message bundle="sheet" key="button.nullity"/>" onclick="forceOperation(2);">
