...
alert(DWZ.msg("statusCode_503")||thrownError);}},dataType:'html'});},
...
var $label=$('<a class="alt" for="'+$this.attr("id")+'">'+$this.attr("alt")+'</a>').appendTo($this.parent());
...
isTag:function(tn){
if(!tn)return false;
return $(this)[0] == null ? false : $(this)[0].tagName.toLowerCase()==tn?true:false;},
...
return this.optional(element)||/((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/.test(value);},"Please specify a valid phone number");
...
if($('#hidecall').css('right')!='0px'){
	rightBarWidth = 0;
}
var cwidth=bar.outerWidth()+$(op.container).outerWidth()-rightBarWidth;
...
bar.show();});
bar.animate({left:5},400,function(){
$(op.splitBar).show();
...
var cwidth=$(op.container).outerWidth()-(cleft-$(op.container).cssv("left"))+rightBarWidth-sbar.outerWidth();
$(op.container).css({left:cleft,width:cwidth});
$(op.collapse,sbar).unbind('click');
$(window).trigger(DWZ.eventType.resizeGrid);
if($('#hidecall').css('right')=='0px'){
	$('#hidecall').animate({right:'-300px'});
}
});
...
function initLayout(){
var iContentW=$(window).width()-(DWZ.ui.sbar?$("#sidebar").width()+10:34)-3;
var iContentH=$(window).height()-$("#header").height()-34;
var rbWidth = 0;
if($("#rightside").length>0){
	rbWidth = 24;
}
$("#container").width(iContentW-rbWidth);
$("#container .tabsPageContent").height(iContentH-0).find("[layoutH]").layoutH();
$("#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy,  #sidebar_r .collapse").height(iContentH+30);
$("#showcall,.accordionContent").height(iContentH);
...
var $this=$(this);
if($this.attr("warn")!=""&& undefined!=$this.attr("warn")){
...
var __ids = _getIds(selectedIds);
if(__ids.split(",").length>1){
	alertMsg.error($this.attr("warn")||DWZ.msg("alertSelectMsgOnlyOne"));
	return false;
}
var url=unescape($this.attr("href")).replace(/{.*}+/g,__ids);
DWZ.debug(url);
if(!url.isFinishedTm()){
...
options.mask=true;eval($this.attr("mask")||"false");
...
if($.fn.ajax2Todo)$("a[target=ajax2Todo]",$p).ajax2Todo();
...
if($("#pagerForm",$p)){
	$("#pagerForm",$p).attr("action",$("form[rel='pagerForm']",$p).attr("action"));
}
if($.fn.pagerForm)$("form[rel=pagerForm]",$p).pagerForm({parentBox:$p});
if($.fn.iselect)$("select[rel=iselect]",$p).iselect();
...
var _themeHref=op.themeBase+"dwz/#theme#/style.css";
return this.each(function(){
...
navTab.closeAllTab();
$("#sidebar").find(".accordion").remove().end().append(html).initUI();},"html");
return false;});});}
function _show($box){
...
this._scrollCurrent();
this._reload($tab);
//$("input:first",navTab.getCurrentPanel()).focus();
},
...
getCurrentTab:function(){
	var jT1=$("div.tabs",navTab.getCurrentPanel());
	var currentIndex = jT1.attr("currentIndex");
	if(!jT1) return jT1;
	var jSelector = jT.add($("> *", jT));
	var jGroups = $("> .tabsContent > *", jSelector);
	var jGroup = jGroups.eq(currentIndex);
	return jGroup;
},
refreshCurrentTab:function(){
	var jT1=$("div.tabs",navTab.getCurrentPanel());
	var currentIndex = jT1.attr("currentIndex");
	var jSelector = jT1.add($("> *", jT1));
	var jTabHeader = $("> .tabsHeader", jSelector);
	var jTabs = $(">.tabsHeaderContent>ul li", jTabHeader);
	var currentA = $("a", jTabs).eq(currentIndex);
	var jGroups = $("> .tabsContent > *", jSelector);
	var jGroup = jGroups.eq(currentIndex);
	jGroup.loadUrl(currentA.attr("href"),{},function(){
		jGroup.find("[layoutH]").layoutH();
		jGroup.attr("loaded",true);
	});
},
refreshCurrentTabByCode:function(){
	var jT1=$("div.tabs",navTab.getCurrentPanel());
	var currentIndex = jT1.attr("currentIndex");
	var jSelector = jT1.add($("> *", jT1));
	var jTabHeader = $("> .tabsHeader", jSelector);
	...
	if(""!=code&&"undifend"!=code){
		url+="?code="+code;
	}
	jGroup.loadUrl(url,{},function(){
		jGroup.find("[layoutH]").layoutH();
		jGroup.attr("loaded",true);
	});
},
...
if(op.currentIndex==iTabIndex){
	...
	var jGroup=jGroups.eq(op.currentIndex);
	var a = $(this).find("a:first").get()[0];
	if(a.href&&!jGroup.attr("loaded"))jGroup.loadUrl(a.href,{},function(){
	jGroup.find("[layoutH]").layoutH();
	jGroup.attr("loaded",true);
	});
...
jT.attr("currentIndex",iTabIndex);
var jSelector=jT.add($("> *",jT));
var jTabHeader=$(op.stTabHeader,jSelector);
var jTabs=$(op.stTab+" li",jTabHeader);
var jGroups=$(op.stTabPanel+" > *",jSelector);
...
return false;}};})(jQuery);(function($){
$.pdialog={
_op:{height:480,width:800,minH:40,minW:50,total:20,max:false,mask:false,resizable:true,drawable:true,maxable:true,minable:true,fresh:true},
_current:null,
_zIndex:42,
getCurrent:function(){
...
$("a.close",dialog).click(function(event){
	if($("#soundControl").parent().html()!=null&&$("#soundControl").attr("src")!=""){
			document.getElementById("soundControl").stop();
	}
$.pdialog.close(dialog);
return false;});
if(op.maxable){
...
var callbackFun = function(data, status){
	...
	if(data && data.appendHtml){
		$form.append(data.appendHtml);
	}
...
dataType:"json",
cache:false,
success:callbackFun||DWZ.ajaxDone,
error:DWZ.ajaxError});}
if(confirmMsg){
...
setTimeout(function(){navTab.closeCurrentTab(json.navTabId);},100);}else if("refreshCurrent"==json.callbackType){
navTab.reload();}else if("forward"==json.callbackType){
navTab.reload(json.forwardUrl);}else if("forwardTOsheet"==json.callbackType){
navTab.openTab("menu_95",json.forwardUrl,{title:"新建工单", fresh:true, external:false});}
else if("forwardConfirm"==json.callbackType){
alertMsg.confirm(json.confirmMsg||DWZ.msg("forwardConfirmMsg"),{
okCall:function(){
navTab.reload(json.forwardUrl);}});}else{
navTab.getCurrentPanel().find(":input[initValue]").each(function(){
$(this).val(json.callbackType);});}
if(json.currentTabId != null && json.currentTabId != ""){
	var start = parseInt(json.currentTabId) ;
	$("#divTab" + (start + 1),navTab.getCurrentPanel()).attr("style" , "") ;
	$(".tabsHeaderContent ul li:eq("+start+")",navTab.getCurrentPanel()).trigger("click");
	$(".tabsHeaderContent ul li:eq("+start+") a",navTab.getCurrentPanel()).trigger("click");
}
if(json.refreshNextTab==true){
...
function tabAjaxDone(json){
	...
	if(true==json.openNavTab){
		var tabId = parseInt(10000*Math.random());
		navTab.openTab(tabId, json.navTabUrl, { title:json.navTabTitle, fresh:false, data:{} });
	}
}

function tabAjaxDoneByCode(json){
	DWZ.ajaxDone(json);
	if(json.statusCode==DWZ.statusCode.ok){
		navTab.refreshCurrentTabByCode();
	}
	if("closeCurrent"==json.callbackType){
		$.pdialog.closeCurrent();
	}
...
/*
function returnOnclickAjaxDone1(json){
...
$(this).val(json.callbackType);});}}
*/
function returnOnclickAjaxDone(json){
DWZ.ajaxDone(json);

if(json.statusCode==DWZ.statusCode.ok){
var type = json.callbackType.split(',');
if (type[0] == 1){
if (type[2]!=0){
$("#import div:[class=imp1]").show();
$("#import input:[name='num0']").val(type[2]);
$("#import input:[name='num6']").val(type[3]);
document.getElementById("pcForm").action='../resource/membersTemp!pcMemberTemp.action?productId='+type[1];
$("#sc").attr("disabled",true);}}
if (type[0] == 2){
$("#import div:[class=imp2]").show();
$("#import input:[name='num1']").val(type[2]);
$("#import input:[name='productIds']").val(type[1]);
document.getElementById("jyForm").action='../resource/membersTemp!jyMemberTemp.action?productId='+type[1];
$("#pc").attr("disabled",true);}
if (type[0] == 3){
$("#import div:[class=imp3]").show();
$("#import input:[name='num2']").val(type[2]);
$("#import input:[name='num3']").val($("#import input:[name='num0']").val());
$("#import input:[name='num4']").val($("#import input:[name='num1']").val());
$("#import input:[name='num5']").val(type[2]);
$("#import input:[name='num7']").val($("#import input:[name='num6']").val());
$("#jy").attr("disabled",true);
document.getElementById("dcForm").action='../resource/membersTemp!exportExcel.action?productId='+type[1];
if (type[2] == 0){$("#dc").attr("disabled",true);}}
if (type[0] == 4){$("#dc").attr("disabled",true);}
$("#progressBar").hide();
}else {$("#progressBar").hide();}}
function returnClickAjaxDone(json){
DWZ.ajaxDone(json);
if(json.statusCode==DWZ.statusCode.ok){
var type = json.callbackType.split(',');
if (type[0] == 1){$("#import1 div:[class=imp1]").show();
$("#import1 input:[name='num0']").val(type[2]);
$("#import1 input:[name='num6']").val(type[3]);
document.getElementById("pc0Form").action='../resource/membersImpLog!pcMemberTemp.action?productId='+type[1]+'&expBatchNum='+type[4];
$("#sc1").attr("disabled",true);}
if (type[0] == 2){
$("#import1 div:[class=imp2]").show();
$("#import1 input:[name='num1']").val(type[2]);
$("#import1 input:[name='productIds']").val(type[1]);
document.getElementById("jy0Form").action='../resource/membersImpLog!jyMemberTemp.action?productId='+type[1]+'&expBatchNum='+type[3];
$("#pc1").attr("disabled",true);}
if (type[0] == 3){
$("#import1 div:[class=imp3]").show();
$("#import1 input:[name='num2']").val(type[2]);
$("#import1 input:[name='num3']").val($("#import1 input:[name='num0']").val());
$("#import1 input:[name='num4']").val($("#import1 input:[name='num1']").val());
$("#import1 input:[name='num7']").val($("#import1 input:[name='num6']").val());
$("#import1 input:[name='num5']").val(type[2]);
$("#import1 input:[id='productId']").val(type[1]);
document.getElementById("dc0Form").action='../resource/membersImpLog!exportExcel.action?productId='+type[1]+'&expBatchNum='+type[3];
$("#jy1").attr("disabled",true);if (type[2] == 0){$("#dc1").attr("disabled",true);}}
if (type[0] == 4){
$("#dc1").attr("disabled",true);}
$("#progressBar1").hide();
}else {$("#progressBar").hide();}}
...
navTabPageBreak(args,json.rel);}
if("closeCurrent"==json.callbackType){
$.pdialog.closeCurrent();}else if ("forwardTOsheet" == json.callbackType) {
$.pdialog.open(json.forwardUrl, "", "新建工单", "width:800px,height:500px");}else{
navTab.getCurrentPanel().find(":input[initValue]").each(function(){
$(this).val(json.callbackType);});}
if(true==json.openNavTab){
	var tabId = parseInt(10000*Math.random());
...
return false;}
function openDialogSearch(form){
var $form=$(form);
var param = "";
$.each($form.serializeArray(),function(index,field){
	if(index==0){
		param += ("?"+field.name+"="+encodeURI(encodeURI(field.value)));
	}else{
		param += ("&"+field.name+"="+encodeURI(encodeURI(field.value)));
	}
});
$.pdialog.open($form.attr("action")+param, '', '',{width:800,height:600,mask:true,mixable:true,minable:true,resizable:true});
return false;}
...
success:$callback,
error:DWZ.ajaxError});}
function ajax2Todo(url,callback){
	var $callback=callback||navTabAjaxDone;
	if(!$.isFunction($callback))$callback=eval('('+callback+')');
	$.ajax({
	type:'POST',
...
function refreshCurrentTab(json){
	var statusCode="200";
    var message="操作成功";
  //  var navTabId="modifyProtInfo_1";
...
ajax2Todo:function(){
	return this.each(function(){
		...
		alertMsg.confirm(title,{
		okCall:function(){
		ajax2Todo(url,$this.attr("callback"));
		}});}else{
		ajax2Todo(url,$this.attr("callback"));}
		event.preventDefault();});});},
dwzExport:function(){
...
var _lookup={currentGroup:"",suffix:"",$target:null,pk:"id",callback:null};/*  这里增加   ,callback:null  modified by henry.xu 20160919 */
var _util={
...
for(var key in args){var name="";
name= (_lookup.pk==key)?_util.lookupPk(key):_util.lookupField(key);
if(name==inputName){
$input.val(args[key]);
if($.browser.msie){//add by DengYouming 2016-05-20
...
break;}}
/*添加回调函数处理;henry.xu 20160919*/
var callback = _lookup['callback'];
try{eval(callback+"(this);");}catch(e){}
});},
...
callback: $this.attr("callback") || null,  /*  这一行是增加的  henry.xu 20160919*/
...
fieldStyle:$th.attr("filedStyle")||"",
fieldAttrs:$th.attr("fieldAttrs")||"",
//modified by henry.xu 添加新增时带回查询参数绑定; 20160923 
bringBackId:$th.attr("bringBackId") || "",
bringBackName:$th.attr("bringBackName")||"",
callback:$th.attr("callback")||"" //回调函数;
};
fields.push(field);});
$tbody.find("a.btnDel").click(function(){
var $btnDel=$(this);
if($btnDel.attr("href").indexOf("javascript:") >= 0){
...
var bringBackName = $this.attr('bringBackName');
if (bringBackName) {$this.attr('bringBackName', bringBackName.replaceSuffix(i));}
...
//modified by henry.xu 添加新增时带回查询参数绑定; 20160923 
html='<input type="hidden" bringBackName="'+field.lookupGroup+'.'+field.bringBackId+'" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'+'<input type="text" bringBackName="'+field.lookupGroup+'.'+field.bringBackName+'" name="'+field.name+'"'+suggestFrag+' lookupPk="'+field.lookupPk+'" size="'+field.size+'" class="'+field.fieldClass+'" readonly="readonly"/>'+'<a class="btnLook" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'"  callback="'+field.callback+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" title="查找带回">查找带回</a>';
break;
case 'attach':
...
break;
case 'date':
html='<input type="text" style="'+field.fieldStyle+'" name="'+field.name+'" readonly="readonly" value="'+field.defaultVal+'" class="date '+field.fieldClass+'" dateFmt="'+field.patternDate+'" size="'+field.size+'" />'+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
break;
case 'date2':
html='<input type="text" style="'+field.fieldStyle+' width:120px" name="'+field.name+'" readonly="readonly" value="'+field.defaultVal+'" class="date '+field.fieldClass+'" dateFmt="'+field.patternDate+'" size="'+field.size+'" />'+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
break;
default:
html='<input type="text" style="'+field.fieldStyle+'" name="'+field.name+'" value="'+field.defaultVal+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+'/>';
break;
case 'num':
html='<input type="text" style="'+field.fieldStyle+'" readonly="readonly" name="'+field.name+'" value="'+field.defaultVal+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+'/>';
break;
case 'more':
html='<select name="sel" class="'+field.fieldClass+'" '+' ><option value=">">大于</option><option value=">=">大于等于</option></select><input type="text" style="width:40px;" name="'+field.name+'" value="'+field.defaultVal+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+'/><select name="sel" style="width:60px;"'+'" class="'+field.fieldClass+'" '+' ><option value=">">小于</option><option value=">=">小于等于</option></select><input type="text" style="width:40px;" name="'+field.name+'" value="'+field.defaultVal+'" size="'+field.size+'" '+attrFrag+'/>';
break;
case 'dict':
html='<select style="width:60px" name="'+field.name+'" value="'+field.defaultVal+'" class="comx3 required"><option value="103080301">飞机</option><option value="103080302">轮船</option><option value="103080303">车</option></select>';
break;
}
...
if((""!=$this.attr("warning"))&&(undefined!=$this.attr("warning"))){
	alertMsg.error($this.attr("warn")||$this.attr("warning"));
	$(this).attr("warning","");
	return false;
}
if(!ids){
...
if($this.attr("reloadUrl")){
	_callback = function(){
		$.pdialog.reload($this.attr("reloadUrl"),{});
		if($this.attr("reloadNavTab")&&undefined !=$this.attr("reloadNavTab")){
		navTab.reloadFlag($this.attr("reloadNavTab"));
	}
}
}
...
var op=$.extend({duration:0,easing:"swing",call:function(){}},options);
return this.each(function(){
var $this=$(this);
			...
			$(this).click(function(){
				if(!$(this).is(":checked")){
					if($(".checkboxCtrl").is(":checked")){
						$(".checkboxCtrl").attr('checked',!$(".checkboxCtrl").is(":checked"));
					}
				}
			})
		})
