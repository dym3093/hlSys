	...
	<div class="searchBar">
		<table class="pageFormContent">
			<tr>
				...
				<td>
					<label>条形码号：</label>
					<input type="text" id="barCode" name="filter_and_barCode_LIKE_S" value="${ filter_and_barCode_LIKE_S }"/>
				</td>		
				<td>
		...
		<c:forEach items="${ page.results }" var="barCodeInfo" varStatus="status">
			<tr target="sid_user" rel="${barCodeInfo.id }">
				...
				<td align="center" nowrap="nowrap">
					   <c:choose>			  	  	  
				  	  	    <c:when test="${ barCodeInfo.isEffective==0 }">条码生成</c:when>
				  	  	    <c:when test="${ barCodeInfo.isEffective==1 }">条码已用</c:when>
