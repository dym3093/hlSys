...
public class CustomerRelationShipService  extends BaseService{
	...
	public void findCustomerRelationShipsByCondition(Page page, CustomerRelationShipQuery custRelaQuery) {
		...
		if(custRelaQuery != null) {
			...
			if(StringUtils.isNotEmpty(custRelaQuery.getCombo())) { 
				jdbcSql.append(" AND instr(ship.COMBO,'").append(custRelaQuery.getCombo().trim()).append("') > 0 "); // modify by Damian 2017-04-18
			...
			if(StringUtils.isNotEmpty(custRelaQuery.getProCode())) {
				jdbcSql.append(" AND (SELECT COUNT(1) FROM HL_CUSTOMER_RELATIONSHIP_PRO pro " +
						" WHERE pro.CUSTOMER_RELATIONSHIP_ID = ship.ID AND instr(pro.PROJECT_CODE,'")
						.append(custRelaQuery.getProCode())
						.append("')>0)>0 ");
			}
    ...
    */
	...
	public void saveCustomerMessage(CustomerRelationShip customerRelationShip, 
		...
		// add Damian 2017-04-26 begin
		String branchCompanyName = customerRelationShip.getBranchCommany().trim();//去掉公司名后面的空格，解决有空字符串情况下拉框无法显示的bug
		customerRelationShip.setBranchCommany(branchCompanyName);
		// add Damian 2017-04-26 end
		...
		if(bankList.size()>0){
			...
			for(CustomerRelationshipLink bank:bankList){
				...
				bank.setCustomerRelationShipName(branchCompanyName);//add Damian 2017-04-26
	...
	public void updateCustomerMessage(CustomerRelationShip customer,
		...
		// add Damian 2017-04-26 begin
		//去掉公司名后面的空格，解决有空字符串情况下拉框无法显示的bug
		String branchCompanyName = customer.getBranchCommany().trim();
		customer.setBranchCommany(branchCompanyName);
		// add Damian 2017-04-26 end
